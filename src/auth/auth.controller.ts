...
export class AuthController {
  ...
  @Post('register')
  async register(@Body() authRegisterInput: AuthRegisterInput): Promise<AuthRegisterOutput> {
      return await this.authService.register(authRegisterInput);
  }
  
  @Post('authenticate')
  async authenticate(@Body() authenticateInput: AuthenticateInput): Promise<AuthenticateOutput> {
      return await this.authService.authenticateUser(authenticateInput);
  }

  @Post('verify-email')
  async verifyEmail(@Body() authConfirmationInput: AuthConfirmationInput):Promise<AuthConfirmationOutput> {
